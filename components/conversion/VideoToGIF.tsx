...
export function VideoToGIF() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const name = files[i].name.replace(/\.[^.]+$/, '.gif')

        results.push({ name, url: gifUrl, size: gifBlob.size })
      ...
      toast.error('Failed to convert video to GIF. Please try again.')
    } finally {
      setConverting(false)
    }
  }
  ...
  const handleDownloadAll = async () => {
    ...
    link.href = url
    link.download = 'video-to-gif.zip'
    link.click()
    URL.revokeObjectURL(url)
