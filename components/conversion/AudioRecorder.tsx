...
export function AudioRecorder() {
  ...
  const handleProcess = async () => {
    ...
    try {
      // Note: AudioRecorder usually doesn't take file input, it uses mic.
      ...
      toast.success('Audio Recorder processed successfully!')
    } catch (error) {
  ...
  const handleDownload = () => {
    if (result) {
      ...
      link.href = result
      link.download = `${file?.name.replace(/\.[^/.]+$/, '') || 'output'}_processed.webm`
      link.click()
    ...
    <div className="space-y-6">
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-200">
          Upload a file to use the Audio Recorder tool.
        </p>
      </div>
