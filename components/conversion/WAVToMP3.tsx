...
export function WAVToMP3() {
  ...
  const handleFiles = (newFiles: File[]) => {
    ...
    if (audioFiles.length === 0) {
      toast.error('Please select WAV files only')
      return
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      console.error('WAV to MP3 error:', error)
      toast.error('Failed to convert WAV to MP3. Please try again.')
    } finally {
      setConverting(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <DropZone onFilesDrop={handleFiles} accept=".wav" multiple />
